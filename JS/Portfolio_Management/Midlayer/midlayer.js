...
const ShowRecommendedSecurityDetails=(recommendedScrip)=>{
  ...
  let recommendationDetailsHtml=`<div class="card">
           ...
           <div class="card-body">
               <h5 class="card-title">${recommendedScrip['Name']}</h5><!--/* 2. Write code here to show the name of the recommended script */-->
  ...
  Utility.AddEventListener('btnAddReco','click',PassRecommendationToForm.bind(this,recommendedScrip));/* 3. Put the appropriate button event */
};
...
const ValidateSecurity=(securityToBeAdded)=>{
  ...
  let {Name, Price, Quantity} = securityToBeAdded;

  if(Name=='NA' || Price=='NA'||Quantity=='NA'){

    ValidationMessage='Name,Price or Quantity cannot be empty';
    return {IsValid,ValidationMessage};
  }else if(SecurityData.filter(i=>i.Name==securityToBeAdded.Name).length>0){/* 6. Find the appropriate array method to replace map to make this work*//*Appropriate method maybe is map */
...
class Do{
  ...
  static DisplaySecurities()
  {
    ...
    let thead=`
                     ...
                     `;/* 7. Identity the appropriate array function which replaces filter method and which works on another array by working on the keys */
  ...
  /* 8. Identify the operator which will display Name,Price,Quantity and FNOAvailable */ 
    ...
    SecuritiesTableControl.innerHTML=`<table>${thead}${tbody}</table>`;
    /* 9. Set this equals to thead and tbody which you are generating above */
