let sum = 5 + 3; // sum=8 
let difference = 5 - 3; // difference=2 
let product = 5 * 3; // product=15 
let division = 5/3; // division=1 
let mod = 5%3; // mod=2 
let value = 5;
value++; // increment by 1, value=6
let value1 = 10;
value1--; // decrement by 1, value=9 
console.log(sum,difference,product,division,mod,value,value1)
let firstName = "James"; 
let lastName = "Roche"; 
let name1 = firstName + " " + lastName;  // name = James Roche 
let strValue="James"; 
let numValue=10; 
let newStrValue= strValue + " " + numValue;  // newStrValue= James 10 
console.log(name1, newStrValue)